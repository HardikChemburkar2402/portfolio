...
/* Minimal data-viz motif behind the hero — a faint plot grid (CSS) with one
   elegant ascending trend line + node points. Sits in the fixed background
   layer, so the hero glass panel blurs it (and the photo stays sharp above). */
function DataMotif() {
  const pts = [
  ...
  return (
    <svg className="motif-chart" viewBox="0 0 1440 810" preserveAspectRatio="xMidYMax slice" aria-hidden="true">
      <defs>
...
function App() {
  ...
  const [contactOpen, setContactOpen] = useAppState(false);

  // Scroll-spy: pick the last section whose top has passed ~40% of the viewport.
  useAppEffect(() => {
    ...
    const onScroll = () => {
      ...
      // snap to the last nav item once the page is scrolled to the bottom

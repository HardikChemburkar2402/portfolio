...
export function Icon({ name, size = 18, stroke = 'currentColor', sw = 1.75, style }) {
  const d = LUCIDE[name] || ''
...
export function BrandIcon({ slug, custom, fallback, size = 30, color = '475569' }) {
  ...
  if (!slug || failed) {
    return <Icon name={fallback || 'box'} size={size} stroke={'#' + color} sw={1.6} />
  }
